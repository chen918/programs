...
for (kk in 1:reps){
  ...
  for (t in 1:5){
    ...
    sigma[2,2]=1.2
    for (j in 1:5){
      tmp=rbind(tmp,abs(rmvnorm(300,mean = c(10+15*(j-1),0.5+1.7*(j-1)),sigma=sigma)))
    }
  ...
  #################################################################################################################
  acodata=data[which(data[,"aco"]==1),]
  nonacodata=data[which(data[,"aco"]==0),]
  #################################################################################################################
  library("mclust")
  library('dplyr')
  
  nonacodata=nonacodata[order(nonacodata[,"hrr"]),]
  clus=nonacodata[,c("hrr","povlt150","XCHaRLSON")]
  head(clus,5)
  
  clus[,c("povlt150","XCHaRLSON")]<-scale(clus[,c("povlt150","XCHaRLSON")],center=TRUE,scale=TRUE)
  head(clus,5)
  
  need<-c()
  for( i in unique(clus[,"hrr"])){
    test<-clus[which(clus[,"hrr"]==i),c("povlt150","XCHaRLSON")]
    test<-as.matrix(test)
 
    m<-Mclust(test,G=1:5,modelNames='VII')
    need<-c(need,paste(100,i,m$classification,sep=""))
  }
  
  nonacodata[,"pracnum"]=need
